...
export class HighResSelectionPlugin extends ToolPluginBase {
  ...
  // ArcGIS REST tiles use {y} with custom TileGrids
private readonly HIGH_RES_URL =
  ...
  private ensureHighResLayer(): void {
    ...
    // 1. Define resolutions for EPSG:4326 (Degrees per pixel)
    // Level 0 for a 180-degree normalized map usually covers 180 degrees in 1 tile (180/256 or 180/512)
    const resolutions = [];
    const maxResolution = 180 / 256; // Standard ArcGIS Mars GCS start
    for (let i = 0; i < 20; i++) {
    ...
    this.highResLayer = new TileLayer({
      source: new XYZ({
        ...
        tileGrid: new TileGrid({
          // The critical fix: ArcGIS GCS origin is the top-left of the projection extent
          origin: [-180, 90], 
